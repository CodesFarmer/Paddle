...
class CollectFpnProposalsOpKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &context) const override {
    auto multi_layer_rois = context.MultiInput<paddle::framework::LoDTensor>("MultiLayerRois");

    auto multi_layer_scores = context.MultiInput<paddle::framework::LoDTensor>("MultiLayerScores");

    auto* fpn_rois = context.Output<paddle::framework::LoDTensor>("FpnRois");

    int post_nms_topN = context.Attr<int>("post_nms_topN");

    PADDLE_ENFORCE_GE(post_nms_topN, 0UL, "The parameter post_nms_topN must be a positive integer");
    ...
    for(int i = 0; i < num_fpn_level; ++i) {
      ...
      PADDLE_ENFORCE(cur_rois_lod.size() == cur_scores_lod.size(),
                        "The length of FPN Rois LoD does not match the Length of FPN Scores LoD");
      ...
      integral_of_all_rois[i + 1] = integral_of_all_rois[i] +
              cur_rois_lod[cur_rois_lod.size() - 1];
    }

    // concatenate all fpn rois scores into a list
    // create a vector to store all scores
    std::vector<ScoreWithID<T> > scores_of_all_rois(integral_of_all_rois[num_fpn_level], ScoreWithID<T>());
    for(int i = 0; i < num_fpn_level; ++i) {
    ...
    std::stable_sort(scores_of_all_rois.begin(), scores_of_all_rois.end(), compare_by_score<T>);
    //for(int i = 0; i < 20; ++i) {
    //  LOG(ERROR)<<scores_of_all_rois[i].score<<","<<scores_of_all_rois[i].batch_id<<","<<scores_of_all_rois[i].index<<","<<scores_of_all_rois[i].level<<" ";
    //}
    ...
    //}
    // sort by batch id
    std::stable_sort(scores_of_all_rois.begin(), scores_of_all_rois.end(), compare_by_batchid<T>);
    //LOG(ERROR)<<"============================================================";
    //for(int i = 0; i < 20; ++i) {
    //  LOG(ERROR)<<scores_of_all_rois[i].score<<","<<scores_of_all_rois[i].batch_id<<","<<scores_of_all_rois[i].index<<","<<scores_of_all_rois[i].level<<" ";
    //}
    ...
    for(int i = 0; i < post_nms_topN; ++i) {
      ...
      memcpy(fpn_rois_data, multi_fpn_rois_data[cur_fpn_level] + cur_level_index*kBoxDim,
             kBoxDim * sizeof(T));
